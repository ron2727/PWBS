...
<?php 
 ...
 <div id="cus">Customer Account Number</div>
...
<?php
$quer = "SELECT * FROM bill WHERE status = 'Pending' ORDER BY id ASC";
$result = mysqli_query($conn,$query);
include 'db2.php';
$id = $_REQUEST['id'];
$result = mysqli_query($conn,"SELECT * FROM bill where owners_id='$id' and status = 'Pending' ");


echo "<div id=\"checkbox-container\">";
echo "<table border='1' class='table' id = 'myTable' bgcolor='#fff' >
...
if($totalcons == 41){
  $bill= (int)$totalcons  * (int)$price + $add41;
    ...
    echo "<tr>";
    echo"<td>  <input type=\"checkbox\"  name=\"checkbox\" value= \"$bill \">  </td>";
  ...
  echo "</div>";
  echo"    <form action=\"./pay/index.php\" method=\"get\"><div class=\"wrapper\">";

echo"<input type=\"text\" id=\"search\" name=\"search\" readonly >

<span type= \"input\"   class=\"recalculate-text text-right text-underline\"  > </span> <INPUT TYPE = \"Submit\"  class=\"btn btn-primary\" id=\"butt\"Name = \"Submit1\" VALUE =\"Pay Now\">
  </form></div>";
...
checkboxContainer.addEventListener('change', e => {
  ...
  searchText.value = formatter.format(total);
});
</script> ";
echo"<script src=\"https://cdnjs.cloudflare.com/ajax/libs/jquery/3.3.1/jquery.min.js\"></script>" ;
echo"<script >    var hi = jQuery(\"#spanval\").text();
$(\"#name\").val(hi);
function myFunction() {
  var name = document.getElementById(\"#spanval\").value;
...
?> 	
    ...
    <span id="total"class="recalculate-text"  NAME='search'></span>
